...
static LRESULT CALLBACK windowProcedure(_In_ HWND window, _In_ UINT message, _In_ WPARAM wparam, _In_ LPARAM lparam) {
	...
	case WM_MOUSEMOVE: {
		...
		if(!client->stream->cursorNorth && !client->stream->cursorEast && !client->stream->cursorSouth && !client->stream->cursorWest) {
			client->stream->cursorNorth = TRUE;
			client->stream->cursorEast = TRUE;
			client->stream->cursorSouth = TRUE;
			client->stream->cursorWest = TRUE;
		}
		...
		SetWindowPos(window, HWND_TOP, bounds.left, bounds.top, bounds.right - bounds.left, bounds.bottom - bounds.top, client->stream->cursorNorth && client->stream->cursorEast && client->stream->cursorSouth && client->stream->cursorWest ? SWP_NOSIZE : (!client->stream->cursorNorth && client->stream->cursorEast) || (client->stream->cursorSouth && !client->stream->cursorWest) ? SWP_NOMOVE : 0);
		break;
	}
	case WM_LBUTTONDOWN:
