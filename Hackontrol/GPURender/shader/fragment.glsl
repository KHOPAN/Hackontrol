...
float de(vec3 p) {
	...
	vec3 pp=p-hpos;
	pp=lookat(point)*pp;
	pp.y-=abs(sin(inputTime))*3.+(fparam-(2.-fparam));
	pp.yz*=rot(-inputTime);
...
vec3 march(vec3 from, vec3 dir) {
	...
	if (d<.001) {
		...
		col+=step(.3,hp)*step(.9,fract(pr.z*.05+inputTime*.5+hp*.1))*.7;
		col+=step(.3,hexpos)*step(.9,fract(zz*.05+inputTime+hexpos*.1))*.3;
	}
	col+=g*.03;
	col.rb*=rot(odir.y*.5);
	return col;
...
void main() {
	vec2 uv = gl_FragCoord.xy / inputSize.xy - 0.5;
	uv.x *= inputSize.x / inputSize.y;
	float time = inputTime * 2.0;
	vec3 from = path(time);

	if(mod(inputTime - 10.0, 20.0) > 10.0) {
		from = path(floor(time / 20.0) * 20.0 + 10.0);
		from.x += 2.0;
	}

	hpos = path(time + 3.0);
	vec3 adv = path(time + 2.0);
	vec3 direction = normalize(vec3(uv, 0.7));
	vec3 dd = normalize(adv - from);
	point = normalize(adv - hpos);
	point.xz *= rot(sin(inputTime) * 0.2);
	direction = lookat(dd) * direction;
	vec3 color = march(from, direction);
