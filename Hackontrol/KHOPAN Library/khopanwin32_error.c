...
/*LPWSTR KHOPANGetErrorMessageWin32(const DWORD code, const LPCWSTR function) {
	return KHOPANInternalGetErrorMessage(code, function, TRUE);
}

LPWSTR KHOPANGetErrorMessageHRESULT(const HRESULT code, const LPCWSTR function) {
	return KHOPANInternalGetErrorMessage(code == S_OK ? ERROR_SUCCESS : (HRESULT) (code & 0xFFFF0000) == MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, 0) ? HRESULT_CODE(code) : ERROR_CAN_NOT_COMPLETE, function, TRUE);
}

LPWSTR KHOPANGetErrorMessageNTSTATUS(const NTSTATUS code, const LPCWSTR function) {
	return KHOPANInternalGetErrorMessage(code, function, FALSE);
}*/
