...
wchar_t* HJ_GetSystemDirectory(const wchar_t* fileName) {
	INT size = GetWindowsDirectoryW(NULL, 0);

	if(size == NULL) {
		HJ_DisplayError(GetLastError(), L"GetWindowsDirectoryW()");
		ExitProcess(-1);
		return NULL;
	}
	...
	if(buffer == NULL) {
		HJ_DisplayError(ERROR_NOT_ENOUGH_MEMORY, L"malloc()");
		ExitProcess(-2);
		return NULL;
	}
